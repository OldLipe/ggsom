...
aes_som <- function(model_som,
  ...
  model_som_values <- data.frame(codes = model_som$data[[1]], id = (1:nrow(model_som$data[[1]])),
  ...
  if (cutree && cutree_value > 0) {
    ...
    model_som_pts$cluster <- cutree(hclust(dist(model_som$codes[[1]])), cutree_value)
    model_som_result <- left_join(model_som_values, model_som_pts, by = "unit.class")
...
sum_unit_class <- function(model_som_values){
  value_result <- data.frame(unit.class=c(1:length(unique(model_som_values$unit.class))))

  for (value in seq_along(unique(model_som_values$unit.class))){
    value_result$sum[value] <- c(model_som_values[model_som_values$unit.class == value, "values"] %>% length())
  }
  return(value_result)
}
